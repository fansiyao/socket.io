            ...
            <div class="addr-list">
              <ul>
                <li v-for="(item,index) in addressList">
                  ...
                  <div class="addr-opration addr-set-default" v-else-if="!item.isDefault" @click="setAddress(item,index)">
  ...
  export default({
    ...
    methods:{
       ...
       setAddress(item,index){
            axios.post('/users/setDefault',{addressId:item.addressId}).then((res)=>{
                this.addressList.forEach((item2,index2)=>{
                    if(index2!=index){
